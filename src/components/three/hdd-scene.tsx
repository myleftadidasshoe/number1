...
/** Procedural stylized HDD built from primitives */
function HDDModel({ scrollRotation }: { scrollRotation: number }) {
  ...
  useFrame((_, delta) => {
    ...
    // Slow idle rotation + scroll-driven rotation
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.5}>
      <group ref={groupRef} rotation={[0.5, 0, 0.1]}>
        ...
        <mesh position={[0, 0.15, -0.8]} rotation={[-0.4, 0, 0]} castShadow>
          <boxGeometry args={[3, 0.05, 2.2]} />
          <meshStandardMaterial color="#16213e" metalness={0.95} roughness={0.2} />
        </mesh>

        {/* Platter - shiny disc */}
        <mesh ref={platterRef} position={[-0.3, 0.05, 0.1]} castShadow>
        ...
        <group ref={armRef} position={[0.9, 0.12, 0.6]}>
          ...
          <mesh>
            ...
            <meshStandardMaterial color="#ff1744" metalness={0.7} roughness={0.4} />
          </mesh>
          {/* Arm body */}
  ...
  return (
    <div ref={containerRef} className="h-full w-full">
      ...
      >
        ...
        <pointLight position={[-3, 2, -2]} intensity={0.8} color="#ff1744" />
        <pointLight position={[3, 1, 3]} intensity={0.6} color="#00e5ff" />
        <spotLight
          position={[0, 6, 0]}
          angle={0.3}
          penumbra={0.8}
        ...
        <ContactShadows position={[0, -1.5, 0]} opacity={0.4} scale={8} blur={2} color="#ff1744" />
